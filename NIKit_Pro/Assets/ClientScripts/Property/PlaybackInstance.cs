...
/// 回放副本对象
/// </summary>
public class PlaybackInstance : InstanceBase
{
    #region 内部接口
    ...
    private IEnumerator DoCreateResource(LPCMapping eventArg)
    {
        ...
        List<KeyValuePair<string, bool>> skillIcons = new List<KeyValuePair<string, bool>>();
        int skillId = 0;
        ...
        foreach (Property ob in FighterList)
        {
            ...
            foreach (LPCValue mks in skills.Values)
            {
                ...
                // 添加技能icon
                skillIcons.Add(new KeyValuePair<string, bool>(
    ...
    private void CreateBatchResource()
    {
        ...
        } else
        {
            ...
            foreach (CsvRow resource in resourceList)
            {
                ...
                // 创建资源
                CreateResource(resource.Query<int>("batch"), resource);
            }
        }
    }
    ...
    /// 创建单个资源
    /// </summary>
    private void CreateResource(int batch, CsvRow data)
    {
        // 必须有class_id_script
        ...
        // 调用脚本参数计算怪物class_id;
        int classId = (int) ScriptMgr.Call(classIdScript, Query<int>("level"), args);
    ...
    private void CreateFighterProperty()
    {
        ...
        foreach(string raw in FormationConst.ALL_RAW_TYPE)
        {
            ...
            for (int i = 0; i < fighters.Count; i++)
            {
                ...
                fighterDbase.Add("is_video_actor", 1);
    ...
    private void WhenReadyCombat(int eventId, MixedValue para)
    {
        ...
        // 开始回合制战斗, 延迟一秒钟开始战斗
        RoundCombatMgr.StartRoundCombat(CombatPropertyList, 0.1f, this, RoundCombatConst.ROUND_TYPE_PLAYBACK);
    }
    ...
    private void WhenRoundCombatEnd(int eventId, MixedValue para)
    {
        ...
        // 不是回放类型
        if (args.GetValue<int>("round_type") != RoundCombatConst.ROUND_TYPE_PLAYBACK)
            return;
    ...
    /// 构造函数
    /// </summary>
    public PlaybackInstance(LPCMapping data) : base(data)
    {
        // 获取副本对象的rid
        string rid = GetRid();
    ...
    public override void DoStart()
    {
        ...
        // 重置时间系统时间缩放
        TimeMgr.DoInitCombatLogic(TimeMgr.TempTimeScale);
    ...
    {
        // 执行副本失败
        DoInstanceClearance(false, RoundCombatConst.END_TYPE_WIN);
    }
    ...
    public void DoInstanceClearance(bool result, int endType)
    {
        ...
        para.Add("id", this.GetRid());

        // 抛出回放结束事件
        EventMgr.FireEvent(EventMgrEventType.EVENT_PLAYBACK_END, MixedValue.NewMixedValue<LPCMapping>(para));
    }
