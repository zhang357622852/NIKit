...
public static class CsvFileMgr
{
...
#if UNITY_EDITOR
    // 包含csv的目录, 只有在csv注释头中有"*"号的列才会被序列化
    static string mCsvFolderPath = Application.dataPath + "/LocalSet";
#endif
    ...
    public static void SaveAll()
    {
        ...
        string[] csvFiles = Directory.GetFiles(mCsvFolderPath, "*.csv");
    ...
    public static CsvFile Deserialize(MemoryStream stream)
    {
        ...
        {
            //LogMgr.Trace("Csv版本 {0} 错误, 最新版本 {1}", ver, CsvFileMgr.Version);
            return null;
        }
...
public class CsvFile
{
    ...
    public void AddRow(int i, CsvRow row)
    {
        ...
        if (pk != null)
        {
            ...
            {
                //LogMgr.Trace("无法添加 {0} 为主key, 主key只能是int或string!", pk.GetDescription());
                return;
            }
        ...
        {
           // LogMgr.Trace("主key {0} 没有值!", mPrimaryKey);
        }
    }
...
public class CsvRow
{
    ...
    public T Query<T>(string columnName, T def = default(T))
    {
        ...
        {
            //LogMgr.Trace("csv序列化失败({0})", mOwner.Name);
            return def;
        }
        ...
        if (mOwner.columns.TryGetValue(columnName, out index))
        {
            ...
            //LogMgr.Trace("{0}.csv 没有列 {1}。", mOwner.Name, columnName);
