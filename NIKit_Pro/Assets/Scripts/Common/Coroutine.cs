...
public class Coroutine : MonoBehaviour
{
    ...
    static void StepFixed(Proxy p)
    {
        ...
        if (! p.IsFinished && p.IsRunning)
        {
            try
            {
                ...
                {
                    LogMgr.Trace("固定协程驱动类型非法, IEnumerator = {0}, Type = {1}",
    ...
    public static Proxy DispatchService(IEnumerator i, UnityEngine.Object owner, string name, bool fixedMode)
    {
        ...
        if (owner == null)
        {
            LogMgr.Trace("owner 是空的，i = {0}, name = {1}", i, name);
            return null;
        }
...
#if UNITY_EDITOR
        if (cm.Count > 500)
            LogMgr.Trace("Coroutine数量太多!");
#endif

        // 没有名字，自动取一个
        if (string.IsNullOrEmpty(name))
            name = Game.GetUniqueName("AnonymousCoroutine");

        // 如果已经包含了该协程名的协程
        if (cm.ContainsKey(name))
        {
            LogMgr.Trace(string.Format("重名Coroutine {0}, 失败!", name));
            return null;
        }
