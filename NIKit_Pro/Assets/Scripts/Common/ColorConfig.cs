...
public static class ColorConfig
{
    ...
    // 装备颜色表
    public static Dictionary<int, string> mEquipColor = new Dictionary<int, string>
    {
        { EquipConst.RARITY_WHITE, "ffffff" },
        { EquipConst.RARITY_GREEN, "82ff75" },
        { EquipConst.RARITY_BLUE, "6ed1ff" },
        { EquipConst.RARITY_PURPLE, "e26eff" },
        { EquipConst.RARITY_DARKGOLDENROD, "ffb26e" },
    };

    // 颜色表
    private static Dictionary<int, string> mColorMap = new Dictionary<int, string>()
    {
        { NC_WHITE, "ffffff" },
        { NC_GREEN, "82ff75" },
        { NC_BLUE, "01d8f8" },
    ...
    {
        // 没有配置该颜色，返回空
        if (!mColorMap.ContainsKey(colorId))
            return string.Empty;

        // 返回配置的颜色
        return mColorMap[colorId];
    }
    ...
    /// 为字符串str添加颜色color
    /// </summary>
    /// <returns>添加了颜色的字符串.</returns>
    /// <param name="str">原始字符串.</param>
    /// <param name="color">ColorConfig.里定义的颜色值常量</param>
    public static string ColorFormat(string str, int color)
    {
        string ret = string.Empty;
        if (!mColorMap.ContainsKey(color))
        {
            LogMgr.Trace("mColorMap没有Key:{0}", color);
            return str;
        }

        ret = string.Format("[{0}]{1}[-]", mColorMap[color], str);
